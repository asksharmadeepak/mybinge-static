...
export default async function BlogPage({ searchParams }: BlogProps) {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-14 md:px-8 md:py-20">
      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#8a8a8a]">Notes</p>
      <h1 className="mt-3 font-heading text-4xl text-white md:text-6xl">Blog</h1>
      <form className="mt-8 grid gap-3 md:grid-cols-[2fr_1fr_auto]">
        <input
          ...
          className="rounded-xl border border-white/[0.08] bg-[#101010] px-4 py-2.5 text-white placeholder:text-[#8a8a8a]"
        />
        <select
          name="category"
          defaultValue={category}
          className="rounded-xl border border-white/[0.08] bg-[#101010] px-4 py-2.5 text-white"
        >
      ...
      <div className="mt-12 divide-y divide-white/[0.06] border-y border-white/[0.06]">
        {current.map((post) => (
          <Link key={post.slug} href={`/blog/${post.slug}`} className="block py-7">
            <p className="text-xs font-medium uppercase tracking-wider text-[#8a8a8a]">
              {post.category ?? "Insights"}
            </p>
            <h2 className="mt-2 font-heading text-2xl text-white md:text-3xl">{post.title}</h2>
            <p className="mt-3 text-sm leading-relaxed text-[#c4c4c4]">{post.description}</p>
            <p className="mt-3 text-xs text-[#8a8a8a]">{post.readingTime}</p>
          </Link>
        ))}
      </div>
